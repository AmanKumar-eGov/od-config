...
public class PaytmGateway implements Gateway {
	...
	private Transaction transformRawResponse(PaytmResponse resp, Transaction currentStatus) {
		...
		try {
			java.util.Date date = new java.text.SimpleDateFormat("yyyy-MM-dd hh:mm:ss.S").parse(resp.getTxnDate());
			if (new java.util.Date().getTime() - date.getTime() > 2 * 60 * 1000) {
				if (resp.getStatus().equalsIgnoreCase("TXN_SUCCESS"))
					status = Transaction.TxnStatusEnum.SUCCESS;
				else if (resp.getStatus().equalsIgnoreCase("TXN_FAILURE"))
					status = Transaction.TxnStatusEnum.FAILURE;
			} else {
				log.info("resp.getTxnDate() {} less than 2 minutes ago with status {} ", resp.getTxnDate(), status);
			}
